...
def main() -> None: # executar a função menu()
    menu()
...
def listar_contas() -> None:
    if len(contas) > 0:
        ...
        for conta in contas: # para cada conta na lista de contas, imprima conta
            print(conta)
            print('--------------------')
            sleep(1)
    else:
    ...
    if len(contas) > 0:
        for conta in contas:
            if conta.numero == numero: 
                c = conta
            # se receber uma conta que exista, c irá receber conta
    return c # se não tiver a conta, irá retornar None
